...
| `/api/unsubscribe` | Optional **GET** (reason survey) + **POST** (confirm) unsubscribe link target (client-facing path; no `/scalemargin/`). Records **`unsubscribed`** with **`metadata.reason`** / **`reason_id`**, and **no email in the URL** (only `uid`, `campaign_id`, `organization_id` from the mail template). POSTs the **same** signed analytics shape as the event pipeline when **`UNSUBSCRIBE_LINK_ANALYTICS_URL`** is set. Survey options: **`UNSUBSCRIBE_REASONS`**; logo: **`LOGO_URL`**. |

With **`pnpm run dev:event-test`**, the child process defaults **`UNSUBSCRIBE_URL_BASE`** to `<EVENT_TEST_PUBLIC_BASE_URL>` (dispatch host base; templates append `/api/unsubscribe` and `/api/preferences`) and **`UNSUBSCRIBE_LINK_ANALYTICS_URL`** to the same capture URL as dispatch’s `metadata.analytics_callback_url`, so link clicks and SendGrid webhooks both produce CSV rows without sending raw PII to ScaleMargin. SendGrid may still emit a separate **`unsubscribe`** webhook after subscription tracking — you may see two rows (link vs webhook); `metadata.source` distinguishes **`unsubscribe_link_click`** from provider-forwarded events.
