...
**Unsubscribe link (browser):** **`GET /api/unsubscribe`** shows a reason survey; **`POST /api/unsubscribe`** records (`src/unsubscribe/link.ts`) — client-facing path without `/scalemargin/`. Same ngrok host pattern as webhooks otherwise. Set **`UNSUBSCRIBE_LINK_ANALYTICS_URL`** (often identical to `analytics_callback_url`) to POST a signed **`unsubscribed`** batch with **`provider: "link_click"`**, **`metadata.source: "unsubscribe_link_click"`**, and **`metadata.reason` / `metadata.reason_id`**. Query/form params: **`uid`**, **`campaign_id`**, **`organization_id`** (no email in the URL). Configure survey options with **`UNSUBSCRIBE_REASONS`** and logo with **`LOGO_URL`**. Dispatch templates: see `config/dispatch.example.yaml`.
