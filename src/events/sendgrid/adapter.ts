...
import { extractCorrelationFromSendGridEvent } from "../common/correlator.js";
import type {
  Correlation,
  InboundEventAdapter,
  SignatureRequest,
  ...
  return {
    ...
    verifySignature(req: SignatureRequest): boolean {
      try {
        ...
        return ew.verifySignature(pubKey, req.rawBody, sig, ts);
      } catch {
        return false;
      }
    },
    ...
    toStandardEvent(stripped: Record<string, unknown>, c: Correlation): StandardizedEvent | null {
      ...
      if (sgEvent === "unsubscribe") metadata.unsubscribe_source = "global";
